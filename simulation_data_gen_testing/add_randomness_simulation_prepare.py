...
2. QP使用的參數準備，儲存給matlab使用 (but resample的部分，給server做)
...
from common_module_pkg import call_r_test
...
# (simulation資訊抽取)
# 1. (全體) (cohort) 1997年的全體人口向量 (cohort的部分包含在QP參數準備當中)
# 2. (全體) (cohort) 1998年的全體人口向量 (cohort的部分包含在QP參數準備當中)
# 3. (cohort) 1997到1998年的改版矩陣 (cohort的部分包含在QP參數準備當中) 
# 4. (cohort) 1997到1998年的習慣矩陣 (全體人口目前還無法計算轉移矩陣)
# 5. (cohort) 1998到1999年的習慣矩陣 (cohort的部分包含在QP參數準備當中) (全體人口目前還無法計算轉移矩陣) 
def simulation_data_info_retrieve(datta_df_list):
    ...
    Input: 
    :datta_df_list: (list) simulation的原始資料

    Output: 
    :first_year_population_vec: (np.array) (4x1) 1997年的全體人口向量
    :second_year_population_vec: (np.array) (5x1) 1998年的全體人口向量
    :third_year_population_vec: (np.array) (5x1) 1999年的全體人口向量
    :first_yr_first_cohort_vec: (np.array) (4x1) 1997年的cohort人口向量
    :second_yr_first_cohort_vec: (np.array) (5x1) 1998年的cohort人口向量 (第一個cohort)
    :second_yr_second_cohort_vec: (np.array) (5x1) 1998年的cohort人口向量 (第二個cohort)
    :third_yr_first_cohort_vec: (np.array) (5x1) 1999年的cohort人口向量
    
    :first_cohort_habbit_matrix: (np.array) (5x5) 1997到1998年的cohort人口習慣矩陣
    :first_pop_ver_change_matrix: (np.array) (5x4) 1997到1998年的全體人口改版矩陣
    :first_cohort_ver_change_matrix: (np.array) (5x4) 1997到1998年的cohort人口改版矩陣
    """
    # 第一組cohort data 
...
def QP_matrix_param_calculate(datta_df_list, threshold_list, second_yr_questionnaire, main_directory):
    """
    Object: 
        估計第一和第二個年份間的改版矩陣

    1. (放在另外的函數) 針對第一年(97年)做gamma fit，並且重新取樣，然後作答第二年的問卷，在原本的問卷填答結果，產生改版矩陣的初始值
    2. 求出97年的人口向量 (重複健檢人口)
    3. 求出98年的人口向量 (重複健檢人口)
    4. 求出97年到98年的總變異矩陣
    5. 求出98年到99年的習慣矩陣
    ...
    :python_c_vec.csv: 97年的人數比例向量
    :python_f_vec.csv: 98年的人數比例向量
    :python_T_matrix.csv: 97到98年的總變異矩陣
    :python_M_matrix.csv: 98到99年的習慣矩陣
    """

    cohort_datta_df = pd.merge(datta_df_list[0], datta_df_list[1], how='inner', on=['id'])

    # 1. 求97年人口向量 (重複健檢人口) (c vector)
...
if __name__ == "__main__": 
    ...
    # print(simulation_datta_df_list)

    # 建立資料夾
    paths = [
        main_directory + 'matrix_and_vector', 
        main_directory + 'qp_input_output'
    ]
    for path in paths: 
    ...
    # 1.1 儲存成csv檔案
    first_year_population_vec.to_csv(main_directory + 'matrix_and_vector/first_year_population_vec.csv', index=False)
    first_year_population_second_year_q_vec.to_csv(main_directory + 'matrix_and_vector/first_year_population_second_year_q_vec.csv', index=False)
    first_cohort_second_year_q_vec.to_csv(main_directory + 'matrix_and_vector/first_cohort_second_year_q_vec.csv', index=False)
    second_year_population_vec.to_csv(main_directory + 'matrix_and_vector/second_year_population_vec.csv', index=False)
    third_year_population_vec.to_csv(main_directory + 'matrix_and_vector/third_year_population_vec.csv', index=False)
    first_cohort_habbit_matrix.to_csv(main_directory + 'matrix_and_vector/first_cohort_habbit_matrix.csv', index=False)
    first_pop_ver_change_matrix.to_csv(main_directory + 'matrix_and_vector/first_pop_ver_change_matrix.csv', index=False)
    first_cohort_ver_change_matrix.to_csv(main_directory + 'matrix_and_vector/first_cohort_ver_change_matrix.csv', index=False)
    first_yr_first_cohort_vec.to_csv(main_directory + 'matrix_and_vector/first_yr_first_cohort_vec.csv', index=False)
    second_yr_first_cohort_vec.to_csv(main_directory + 'matrix_and_vector/second_yr_first_cohort_vec.csv', index=False)
    second_yr_second_cohort_vec.to_csv(main_directory + 'matrix_and_vector/second_yr_second_cohort_vec.csv', index=False)
    third_yr_first_cohort_vec.to_csv(main_directory + 'matrix_and_vector/third_yr_first_cohort_vec.csv', index=False)
    ...
    first_yr_cohort_vec, second_yr_cohort_vec, first_total_transition_matrix, second_habbit_transition_matrix = \
        QP_matrix_param_calculate(simulation_datta_df_list, simulation_data_threshold_list, gamma_generater.questionnaire_1998, main_directory)
    print('第一年的cohort向量: \n', first_yr_cohort_vec)

    # 2.1 存成csv檔案
    first_yr_cohort_vec.to_csv(main_directory + '/matrix_and_vector/first_yr_cohort_vec.csv', index=False)
    second_yr_cohort_vec.to_csv(main_directory + '/matrix_and_vector/second_yr_cohort_vec.csv', index=False)
    first_total_transition_matrix.to_csv(main_directory + '/matrix_and_vector/first_total_transition_matrix.csv', index=False)
    second_habbit_transition_matrix.to_csv(main_directory + '/matrix_and_vector/second_habbit_transition_matrix.csv', index=False)
